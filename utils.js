...
exports.runDrv = (power) => {
    let drvLink = feedList.find(feed => feed.name === "drv")
    
    adafruit.publish(drvLink, JSON.stringify({
        id: 10,
        name: "DRV_PWM",
        data: Math.round(power * 2.55).toString(),
        unit: ""
    }))
}

exports.getHumidityLimit = async () => {
    let queryCommand = "SELECT upper_bound, lower_bound FROM farm.sensor WHERE system_id = '101'"
    let {upper_bound: top, lower_bound: bottom} = (await query(dbConn, queryCommand))[0]
    return {top, bottom}
}
